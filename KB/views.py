...
def changepassword(request,pk):
	msg="your old password donot match.you will only get 5 chances.."
...
def invoice(request):
	msg="Available quantity is less.So you need to reduce your quantity per Kg."
	...
	if request.method=='GET':
		...
		upk=request.GET['upk']
		cpk=request.GET['cpk']
		avlquan=request.GET['avlquantity']
	...
	if request.method=='POST':
		...
		if float(avlquan)<float(Quantity):
			return render(request,'morequan.html',{'uname':uname,'userdetails':userdetails,'msg':msg})
		else:
			...
			pquan=Product.objects.get(id=ppk,active='1')
			pquan.pquantity=int(pquan.pquantity)-int(Quantity)
			pquan.save()
		...
		#html_content = '<p>This is to inform you to that you have to <strong>deliver</strong> product.</p>'
				...
				return render(request,'morequan.html',{'uname':uname,'userdetails':userdetails,'msg':msg1})
